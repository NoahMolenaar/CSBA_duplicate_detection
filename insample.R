...
# Load the JSON data
data <- fromJSON("~/OneDrive/Noah laptop/Erasmus Universiteit Rotterdam/Master jaar 1 (2023-2024)/Blok 2/Computer Science for Business Analytics/TVs-all-merged.json")
...
new_combined_df$title <- gsub('-inch', 'inch', new_combined_df$title)
new_combined_df$title <- gsub('-inches', 'inch', new_combined_df$title)
new_combined_df$title <- gsub(' inches', 'inch', new_combined_df$title)
new_combined_df$title <- gsub('hertz', 'hz', new_combined_df$title)
...
television_brands <- read_excel("Library/CloudStorage/OneDrive-Persoonlijk/Noah laptop/Erasmus Universiteit Rotterdam/Master jaar 1 (2023-2024)/Blok 2/Computer Science for Business Analytics/television brands.xlsx")
...
band_values <- c(20, 25, 40, 50, 80, 100, 125, 200, 400)
...
# Assuming average_results_df is your data frame
row_to_swap1 <- 7
row_to_swap2 <- 8

# Store the rows to be swapped
row1 <- average_results_df[row_to_swap1, ]
row2 <- average_results_df[row_to_swap2, ]

# Replace the rows
average_results_df[row_to_swap1, ] <- row2
average_results_df[row_to_swap2, ] <- row1


# Assuming you have the data stored in variables Avg_PC_LSH, Avg_PQ_LSH, and Avg_Fraction_Comparisons

# Plot Avg_PC_LSH against Avg_Fraction_Comparisons
plot(average_results_df$Avg_Fraction_Comparisons, average_results_df$Avg_PC_LSH[], type = "l", col = "blue", xlab = "Fraction Comparisons", ylab = "PC", main = "Average PC LSH vs. Average Fraction Comparisons")

# Plot Avg_PQ_LSH against Avg_Fraction_Comparisons
plot(average_results_df$Avg_Fraction_Comparisons, average_results_df$Avg_PQ_LSH[-7], type = "l", col = "red", xlab = "Fraction Comparisons", ylab = "PQ", main = "Average PQ LSH vs. Average Fraction Comparisons")
